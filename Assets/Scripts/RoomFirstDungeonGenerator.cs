...
public class RoomFirstDungeonGenerator : SimpleWalkGenerator
{
    ...
    private Vector2Int _safeStartPosition = Vector2Int.zero;
    private Vector2Int _portalPosition = Vector2Int.zero;
    public List<Vector2Int> enemySpawnPositions = new();
    ...
    {
        return _safeStartPosition;
    }

    public Vector2Int FindPortalSpawnLocation()
    {
        return _portalPosition;
    }

    public List<Vector2Int> FindEnemySpawnLocations()
    {
    ...
    {
        var roomList = ProceduralGenerationAlgos.BinarySpacePartition(new BoundsInt((Vector3Int)startPosition, new Vector3Int(dungeonWidth, dungeonHeight, 0)), minRoomWidth, minRoomHeight);
        ...
        foreach (var room in roomList)
        {
            ...
            enemySpawnPositions.Add((Vector2Int)Vector3Int.RoundToInt(room.center));
        }

        var playerSpawn = Random.Range(0, roomCenters.Count);
        // choose a random room to start from
        var randomRoomCenter = roomCenters[playerSpawn];
        var portalRoomCenter = roomCenters[Random.Range(0, roomCenters.Count)];

        while (randomRoomCenter == portalRoomCenter)
        {
            portalRoomCenter = roomCenters[Random.Range(0, roomCenters.Count)];
        }

        _safeStartPosition = randomRoomCenter;
        _portalPosition = portalRoomCenter;
        enemySpawnPositions.RemoveAt(playerSpawn);
